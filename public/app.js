...
let state={room:null,nickname:'',selected:'ladder',topic:'',bingoSize:5,countdown:null,dodgeX:50,dodgeTimer:0,keys:{},spectateId:null,raceKeyDown:false,timingKeyDown:false,typingComposing:false,typingRaf:0};
let bingoDraft=null;
let bingoComposing=false;
const gameMeta={ladder:['🪜','사다리 타기'],bingo:['⭕','빙고'],dodge:['💩','똥피하기'],race:['🏎️','레이싱'],timing:['⏱️','타이밍 게임'],liar:['🕵️','라이어 게임'],bomb:['💣','폭탄 돌리기'],memory:['🃏','카드 뒤집기'],typing:['⌨️','타자게임'],waterball:['💦','물풍선 대전'],gomoku:['⚫','오목']};
...
function desc(k){return {ladder:'운명을 따라 내려가기',bingo:'3×3 · 4×4 · 5×5 선택',dodge:'실시간 생존 게임',race:'스페이스바 연타 대결',timing:'5~10초 랜덤 타이밍 맞추기',liar:'3라운드 설명 후 라이어 찾기',bomb:'폭탄을 넘기고 끝까지 생존',memory:'같은 그림을 찾아 점수 대결',typing:'내려오는 단어를 가장 먼저 입력',waterball:'물풍선·아이템 실시간 대전',gomoku:'두 명이 겨루는 오목'}[k]}
...
function gameView(){const r=state.room;return {ladder:ladderView,bingo:bingoView,dodge:dodgeView,race:raceView,timing:timingView,liar:liarView,bomb:bombView,memory:memoryView,typing:typingView,waterball:waterballView,gomoku:gomokuView}[r.game]()}
...
function afterRender(){if(state.room?.phase==='countdown')runCountdown();if(state.room?.game==='dodge'&&state.room.phase==='playing')startDodgeLocal();if(state.room?.game==='typing'&&state.room.phase==='playing')startTypingLocal()}
...
function waterballItemIcon(t){return t==='range'?'🔥':t==='bomb'?'➕':'⚡'}
function waterballView(){
  const r=state.room,w=r.waterball||{},wp=w.players?.[socket.id];
  if(r.phase==='lobby')return `<div class="center"><div class="title">💦 물풍선 대전</div><p class="sub">방향키 이동 · Space 물풍선 · 블록을 부수고 아이템을 먹어 마지막까지 살아남으세요!</p><div class="water-rules"><span>🔥 물줄기 증가</span><span>➕ 풍선 개수 증가</span><span>⚡ 이동속도 증가</span></div></div>${host()?'<div class="actions" style="justify-content:center"><button class="btn primary" onclick="socket.emit(\'waterball:start\')">물풍선 대전 시작</button></div>':'<p class="center sub">방장이 시작할 때까지 기다려 주세요.</p>'}`;
  ...
  for(let rr=0;rr<rows;rr++)for(let cc=0;cc<cols;cc++){
    ...
    cells.push(`<div class="water-cell ${tile} ${flame?'flame':''}">${tile==='hard'?'🧱':tile==='soft'?'📦':''}${item?`<span class="water-item">${waterballItemIcon(item.type)}</span>`:''}${bomb?'<span class="water-bomb">💧</span>':''}${flame?'<span class="water-flame">💦</span>':''}${players.map(p=>`<span class="water-player ${p.alive?'':'dead'} ${p.id===socket.id?'mine':''}" style="--pc:${p.color}" title="${esc(p.nickname)}"><i>🙂</i><b>${esc(p.nickname.slice(0,4))}</b></span>`).join('')}</div>`);
  }
  return `<div class="water-head"><div><div class="title">💦 물풍선 대전</div><div class="sub">방향키 이동 · Space 물풍선 설치</div></div>${wp?`<div class="water-stats"><span>💧 ${wp.maxBombs}</span><span>🔥 ${wp.range}</span><span>⚡ ${wp.speed}</span></div>`:''}</div>${wp&&!wp.alive?'<div class="spectator-panel"><b>💥 탈락!</b><span>남은 참가자들의 대전을 관전 중입니다.</span></div>':''}<div class="water-board" style="--cols:${cols}">${cells.join('')}</div><div class="water-legend"><span>📦 부술 수 있는 블록</span><span>💧 설치한 물풍선</span><span>💦 물줄기에 닿으면 탈락</span></div>`;
}
