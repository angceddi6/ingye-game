...
function catchmindView(){
  ...
  if(r.phase==='lobby')return `<div class="center"><div class="title">🎨 캐치마인드</div><p class="sub">한 명씩 그림을 그리고 나머지 참가자들은 채팅으로 정답을 맞혀요.</p><div class="catch-rules"><b>${c.rounds||3}라운드</b> · 제시어 선택 10초 · 그림 제한시간 45초 · 후반부 글자 힌트 공개</div>${host()?`<div class="field catch-round-lobby"><label>진행 라운드</label><div class="size-picker">${[3,4,5].map(n=>`<button class="size-option ${c.rounds===n?'selected':''}" onclick="socket.emit('catchmind:rounds',${n})">${n}라운드</button>`).join('')}</div></div><button class="btn primary" onclick="socket.emit('catchmind:start')">캐치마인드 시작</button>`:'<p class="sub">방장이 게임을 시작할 때까지 기다려 주세요.</p>'}</div>`;
