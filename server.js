...
const CATCHMIND_CHOOSE_MS = 10_000;
const CATCHMIND_DRAW_MS = 80_000;
const CATCHMIND_HINT_DELAYS = [41_000,54_000,67_000];
...
function beginCatchmindDrawing(room,word){const c=room.catchmind;if(c.stage!=='choosing')return;c.stage='drawing';c.word=word;c.wordLength=catchWordLength(word);c.chooseDeadline=null;c.drawDeadline=Date.now()+CATCHMIND_DRAW_MS;c.revealed=[];c.hintMask=makeCatchMask(word,[]);c.guesses=[];c.chat=[];c.strokes=[];emitRoom(room);
  ...
  CATCHMIND_HINT_DELAYS.forEach((delay,i)=>revealAt(delay,i+1));room.timeouts.push(setTimeout(()=>finishCatchmindTurn(room),CATCHMIND_DRAW_MS+100));
}
